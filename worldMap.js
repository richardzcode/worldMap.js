...
var Map = function() {
  ...
  var defaultStyle = { background: { stroke: '#090C5C', fill: '#090C5C' }
      , land: { stroke: '#3C3F96', fill: '#3C3F96' }
      };

  this.latLngToPoint = function (lat, lng, mapArea) {
    var x = (lng / 360) + 0.5;
    var y = 1 - ((lat / 180) + 0.5);
    
    return { x: Math.floor(x * mapArea.width) + mapArea.x
           , y: Math.floor(y * mapArea.height) + mapArea.y};
  };
  
  this.drawBackground = function(context, backgroundStyle, mapArea) {
    var backgroundStyle = backgroundStyle || defaultStyle.background
      , mapArea = mapArea || { x: 0, y: 0, width: context.canvas.width, height: context.canvas.height };;
      
    context.strokeStyle = backgroundStyle.stroke;
  ...
  this.drawLand = function(context, land, landStyle, mapArea) {
    var landStyle = landStyle || defaultStyle.land
      , mapArea = mapArea || { x: 0, y: 0, width: context.canvas.width, height: context.canvas.height };
    
    context.strokeStyle = landStyle.stroke;
    context.fillStyle = landStyle.fill;
    context.beginPath();
    
    var dot = land[0]
      , point = this.latLngToPoint(dot[1], dot[0], mapArea);
    context.moveTo(point.x, point.y);
    
    for (var j = 1; j < land.length; j ++) {
      dot = land[j];
      point = this.latLngToPoint(dot[1], dot[0], mapArea);
  ...
  *                   , land: { stroke: 'stroke style', fill: 'fill style' }
  *                   }
  *          }
  */
  this.draw = function(canvas, options, callback) {
    var context = canvas.getContext('2d')
      , options = options || {}
      , mapArea = options.mapArea || { x: 0, y: 0, width: canvas.width, height: canvas.height }
      , style = options.style || {}
      , backgroundStyle = style.background || defaultStyle.background
      , landStyle = style.land || defaultStyle.land;
      
    backgroundStyle.stroke = backgroundStyle.stroke || backgroundStyle.fill;
    landStyle.fill = landStyle.fill || landStyle.stroke;
      
    this.drawBackground(context, backgroundStyle, mapArea);
